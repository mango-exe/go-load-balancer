...
func (l *LoadBalancer) parseUrls() {
	var wg sync.WaitGroup
	for idx, rawUrl := range l.rawUrls {
		wg.Add(1)
...
func BuildLoadBalancer() *LoadBalancer {
	...
	var loadBalancerType LoadBalancerType
	switch config.Type {
	case "roundrobin":
	...
	loadBalancer := LoadBalancer{
		stickySessionEnabled: config.StickySessions,
		healthChecksEnabled:  config.HealthCheck,
		tlsEnabled:           config.Tls,
		rateLimitEnabled:     config.RateLimit,
		rawUrls:              config.Servers,
